...
import { findCursorDirs, parseCursorSessions } from './parser-cursor';
  ...
  },
  {
    ...
    collectSync(ctx) {
      for (const cursorDir of findCursorDirs()) {
        for (const session of parseCursorSessions(cursorDir)) addSession(ctx.workspaces, ctx.sessions, session, cursorDir);
      }
    },
  },
];
